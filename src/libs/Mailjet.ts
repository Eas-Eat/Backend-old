...
export function sendEmailWithTemplate(Recipients: Array<Recipient>, templateId: number): Promise<Response> {
  const { MJ_APIKEY_PUBLIC, MJ_APIKEY_PRIVATE } = process.env
  const mailjet = connect(MJ_APIKEY_PUBLIC as string, MJ_APIKEY_PRIVATE as string)
